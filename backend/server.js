...
app.get(
  ...
  (req, res) => {
    const token = jwt.sign({ id: req.user.id, email: req.user.email }, SECRET, {
      expiresIn: '1h',
    })
  ...
  try {
    const doc = await prisma.document.create({
      data: {
        ...
        ownerId: req.user.id,
        pages: {
          create: [{ pageIndex: 0, content: '<p>Start typing...</p>' }],
        },
      },
    })
...
app.delete(
  '/api/documents/:docId/pages/:pageIndex',
  authenticate,
  async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        ...
        esErr.meta?.body?.error || esErr.message
      )
    }
...
// [NEW] Reorder document pages
app.patch(
  '/api/documents/:docId/pages/reorder',
  authenticate,
  async (req, res) => {
    const { docId } = req.params
    ...
    try {
      ...
      const document = await prisma.document.findUnique({
        where: { id: docId },
        include: { owner: true, shares: true, pages: true },
      })
      if (!document)
        return res.status(404).json({ error: 'Document not found' })
      const userId = req.user.id
      ...
      // Validate all pageIds belong to this document
      const pagesSet = new Set(document.pages.map((p) => p.id))
      for (const pageId of order) {
        if (!pagesSet.has(pageId)) {
          return res
            .status(400)
            .json({ error: `Page ${pageId} does not belong to document` })
        }
      }
      // No extra pageIds
      if (order.length !== document.pages.length) {
        return res
          .status(400)
          .json({ error: 'Order does not match number of pages' })
      }
      // Update pageIndex in order
      await prisma.$transaction(
        order.map((pageId, idx) =>
          prisma.page.update({
            where: { id: pageId },
            data: { pageIndex: idx },
          })
      ...
      // Broadcast to all websockets in this doc
      if (global.documentClients && global.documentClients[docId]) {
        for (const ws of global.documentClients[docId]) {
          if (ws.readyState === 1) {
            ws.send(JSON.stringify({ type: 'pages-reordered', data: order }))
          }
        }
      }
      res.json({ success: true })
    } catch (error) {
      console.error('Reorder pages failed:', error)
