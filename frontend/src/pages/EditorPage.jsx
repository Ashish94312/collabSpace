  ...
  useEffect(() => {
      ...
      editorRef.current.addEventListener('applySyntaxHighlighting', (e) => {
        ...
        switch (language) {
          ...
          default:
            ...
            highlightedCode = codeBlock.textContent.replace(/[<>&]/g, (match) => {
              switch (match) {
                case '<': return '&lt;';
                case '>': return '&gt;';
                case '&': return '&amp;';
    ...
    return () => {
      if (editorRef.current) {
        ...
        editorRef.current.removeEventListener('applySyntaxHighlighting', () => {});
      }
    };
  }, [editorRef]);
...
// Add your code here`;
  ...
  const highlightJavaScript = (code) => {
    return code
      .replace(/\b(const|let|var|function|class|if|else|for|while|try|catch|async|await|return|import|export|from|default)\b/g, '<span class="keyword">$1</span>')
      .replace(/\b(console|fetch|JSON|Promise|Error|Date|Math|Array|Object|String|Number|Boolean)\b/g, '<span class="function">$1</span>')
  ...
  const highlightPython = (code) => {
    return code
      .replace(/\b(def|class|if|else|elif|for|while|try|except|finally|with|import|from|as|return|True|False|None|async|await)\b/g, '<span class="keyword">$1</span>')
      .replace(/\b(print|len|str|int|float|list|dict|set|tuple|range|enumerate|zip|map|filter|reduce)\b/g, '<span class="function">$1</span>')
      .replace(/(#.*$)/gm, '<span class="comment">$1</span>')
      .replace(/("""[\s\S]*?""")/g, '<span class="comment">$1</span>')
      .replace(/('''[\s\S]*?''')/g, '<span class="comment">$1</span>')
      .replace(/(['"])((?:\\.|(?!\1)[^\\])*?)\1/g, '<span class="string">$1$2$1</span>')
  ...
  const highlightCSS = (code) => {
    return code
      .replace(/([.#]?[a-zA-Z][a-zA-Z0-9_-]*)\s*{/g, '<span class="selector">$1</span> {')
      .replace(/([a-zA-Z-]+):/g, '<span class="property">$1</span>:')
      .replace(/(["'])((?:\\.|(?!\1)[^\\])*?)\1/g, '<span class="string">$1$2$1</span>')
      .replace(/\b(url|rgb|rgba|hsl|hsla|calc|var)\b/g, '<span class="function">$1</span>')
      .replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="comment">$1</span>');
  };

  const highlightReact = (code) => {
    return code
      .replace(/\b(const|let|var|function|class|if|else|for|while|try|catch|async|await|return|import|export|from|default|useState|useEffect|useCallback|useMemo|React)\b/g, '<span class="keyword">$1</span>')
      .replace(/\b(console|fetch|JSON|Promise|Error|Date|Math|Array|Object|String|Number|Boolean|createPortal|memo)\b/g, '<span class="function">$1</span>')
      .replace(/(\/\/.*$)/gm, '<span class="comment">$1</span>')
      .replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="comment">$1</span>')
      .replace(/(['"`])((?:\\.|(?!\1)[^\\])*?)\1/g, '<span class="string">$1$2$1</span>')
      .replace(/\b(\d+(?:\.\d+)?)\b/g, '<span class="number">$1</span>')
      .replace(/(&lt;\/?)([a-zA-Z][a-zA-Z0-9]*)([^&]*?)(&gt;)/g, '<span class="jsx-tag">$1$2$3$4</span>')
      .replace(/(\s[a-zA-Z-]+)=/g, '<span class="jsx-attribute">$1</span>=');
  };

  const highlightSQL = (code) => {
    return code
      .replace(/\b(SELECT|FROM|WHERE|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|TABLE|INDEX|PRIMARY|FOREIGN|KEY|REFERENCES|UNIQUE|NOT|NULL|DEFAULT|CHECK|CONSTRAINT|WITH|AS|JOIN|LEFT|RIGHT|INNER|OUTER|ON|GROUP|BY|ORDER|HAVING|LIMIT|OFFSET|UNION|ALL|DISTINCT|COUNT|SUM|AVG|MAX|MIN|CASE|WHEN|THEN|ELSE|END|AND|OR|IN|EXISTS|BETWEEN|LIKE|IS|CASCADE|RESTRICT|SET|NULL|TRUE|FALSE|CURRENT_TIMESTAMP|IDENTITY|BIGINT|VARCHAR|TEXT|BOOLEAN|TIMESTAMP|TIMEZONE|DECIMAL|ROUND|ROW_NUMBER|OVER|PARTITION|BY)\b/gi, '<span class="keyword">$1</span>')
      .replace(/\b(COUNT|SUM|AVG|MAX|MIN|ROUND|ROW_NUMBER|OVER|PARTITION|BY)\b/gi, '<span class="function">$1</span>')
      .replace(/(--.*$)/gm, '<span class="comment">$1</span>')
      .replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="comment">$1</span>')
      .replace(/(['"])((?:\\.|(?!\1)[^\\])*?)\1/g, '<span class="string">$1$2$1</span>')
      .replace(/\b(\d+(?:\.\d+)?)\b/g, '<span class="number">$1</span>')
  ...
  const highlightBash = (code) => {
    return code
      .replace(/\b(if|then|else|elif|fi|for|while|do|done|case|esac|function|return|exit|break|continue|echo|printf|read|export|source|\.|cd|ls|cp|mv|rm|mkdir|rmdir|chmod|chown|grep|sed|awk|sort|uniq|head|tail|cat|less|more|nano|vim|git|npm|node|python|bash|sh|zsh)\b/g, '<span class="keyword">$1</span>')
      .replace(/\b(echo|printf|read|export|source|cd|ls|cp|mv|rm|mkdir|rmdir|chmod|chown|grep|sed|awk|sort|uniq|head|tail|cat|less|more|nano|vim|git|npm|node|python|bash|sh|zsh)\b/g, '<span class="function">$1</span>')
      .replace(/(#.*$)/gm, '<span class="comment">$1</span>')
      .replace(/(['"])((?:\\.|(?!\1)[^\\])*?)\1/g, '<span class="string">$1$2$1</span>')
      .replace(/\b(\d+(?:\.\d+)?)\b/g, '<span class="number">$1</span>')
    ...
    const handleKeyDown = (e) => {
      if (e.key === 'Enter') {
        ...
        while (currentNode && currentNode !== document.body) {
          if (currentNode.nodeType === Node.ELEMENT_NODE && currentNode.tagName === 'PRE') {
            pre = currentNode;
            break;
          }
